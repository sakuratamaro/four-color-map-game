...
test("combined pilot colored corner capability accepts alpha4, alpha5 and new-card alpha6",()=>{
 const source=app.match(/function supportsColoredCornerBloom\(state\) \{[^}]+\}/)?.[0];
 assert.ok(source);
 const supports=require("node:vm").runInNewContext("("+source+")");
 for(const version of ["5.0.0-alpha.4","5.0.0-alpha.5","5.0.0-alpha.6"])assert.equal(supports({engineVersion:version}),true,version);
 for(const version of [undefined,null,"","5.0.0-alpha.1","5.0.0-alpha.2","5.0.0-alpha.3","5.0.0-alpha.7","5.0.0"])assert.equal(supports({engineVersion:version}),false,String(version));
...
test("combined pilot identifies new assets separately from frozen navigation and CPU parents",()=>{
 for(const marker of ["app.js?v=20260921-2","style.css?v=20260921-1","standard-online-client.js?v=20260914-1","standard-skill-registry.generated.js?v=20260914-2"]){
  const current=marker==="app.js?v=20260921-2"?"app.js?v=20260926-1"
    :marker==="standard-skill-registry.generated.js?v=20260914-2"?"standard-skill-registry.generated.js?v=20260926-1":marker;
 ...
 for(const marker of ["play-surface-model.js?v=20260915-1","result-continuation.js?v=20260914-2","cpu-progression-model.js?v=20260914-1"]){
  const current=marker==="cpu-progression-model.js?v=20260914-1"?"cpu-progression-model.js?v=20260926-1":marker;
  assert.ok(app.includes(current),current);assert.ok(spec.includes(marker),marker);
 }
 const localMarker="app.bundle.js?v=20260914-11-63d4f2b526f1";
 assert.ok(read("standard-v5/index.html").includes("app.bundle.js?v=20260926-1"));assert.ok(spec.includes(localMarker));
