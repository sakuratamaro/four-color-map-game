...
test("ticket levels use the approved rarity floors and visible long-shot chances", () => {
  ...
  for (const level of [1, 2, 3, 4, 5]) {
    assert.equal(Object.values(gacha.GACHA_ODDS[level]).reduce((sum, value) => sum + value, 0), 100, `Lv.${level}`);
  }
...
test("drawing adds cards without replacing or decrementing the existing inventory", () => {
  const root = fixture(8272);
  root.profiles.playerA.inventory = { colorPrism: 2, areaMicroBloom: 3 };
  const before = JSON.parse(JSON.stringify(root.profiles.playerA.inventory));
  const result = gacha.drawGacha(args(root, { count: 1 }));
  assert.equal(result.ok, true);
  for (const [skillId, count] of Object.entries(before)) {
    assert.ok(result.root.profiles.playerA.inventory[skillId] >= count, skillId);
  }
  assert.deepEqual(root.profiles.playerA.inventory, before, "caller inventory remains immutable");
  assert.equal(Object.values(result.root.profiles.playerA.inventory).reduce((sum, count) => sum + count, 0), 6);
});
