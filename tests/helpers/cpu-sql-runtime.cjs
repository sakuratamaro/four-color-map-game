"use strict";
// Default: isolated PostgreSQL/WASM without a URL or persistent dataDir.
// Optional nativeDatabase is an owned local test cluster, never a production URL.
...
async function createCpuSqlDatabase({ nativeDatabase = null } = {}) {
  const db = nativeDatabase || new PGlite();
  try {
    if (nativeDatabase) console.error("PG_TEST_SQL_BOOTSTRAP");
    ...
    await db.exec(`
      ...
      ${nativeDatabase ? "" : `create function extensions.digest(value text, algorithm text) returns bytea language sql immutable as
    ...
    for(const name of baseline){
      if (nativeDatabase) console.error("PG_TEST_MIGRATION " + name);
      ...
      if (!nativeDatabase) sql=sql.replace(/^create extension if not exists pgcrypto with schema extensions;\r?\n/m,"");
