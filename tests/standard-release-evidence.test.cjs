...
test("release evidence ledger keeps local, public, approval, and blocked states distinct", () => {
  for (const state of ["VERIFIED", "BLOCKED", "PENDING_APPROVAL", "PENDING", "NOT_RUN"]) assert.match(evidence, new RegExp(state));
  for (const gate of ["採否棚卸し", "ローカル製品試験", "Dashboard Advisor", "migration 006–013本番適用", "Edge Function更新", "GitHub main・Pages更新", "別々の二端末による最終受入"]) {
