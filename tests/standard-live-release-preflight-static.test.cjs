...
test("release preflight is read-only, secret-free, finite, and stage-aware", () => {
  ...
  assert.match(source, /cpu-portraits\.js\?v=20260907-1/);
  assert.match(source, /getOptionalText\(`\$\{publicUrl\}cpu-portraits\.js`\)/);
  assert.match(source, /portraitManifest\.status === 200/);
  assert.match(source, /portraitFiles\.length === 49/);
  assert.match(source, /portraitAssetStatuses\.every\(\(status\) => status === 200\)/);
...
test("candidate page and app satisfy the alpha.4 cache generation marker", () => {
  ...
  assert.equal(candidateHtml.includes("cpu-portraits.js?v=20260907-1"), true);
