...
const resultContinuation=fs.readFileSync(path.join(root,"standard-online-v5/result-continuation.js"),"utf8");
test("UDL062 normal connection and recovery copy uses outcomes, not transport identifiers",()=>{
  ...
  assert.match(app,/terminalRewardPresentation\(roomModel\?\.room, mySeat, profile\(\)\)/);
  assert.match(resultContinuation,/報酬を確認中です。/);
