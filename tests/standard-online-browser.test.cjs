...
test("actual browser presents CPU commentary once from public events and keeps terminal reasons visible", { timeout: 120000 }, async () => {
  await withPage("cpuCommentary", async (page) => {
    ...
    await page.locator("#cpuTerminalPortraitOverlay").waitFor({ state: "visible", timeout: 5000 });
    assert.match(await page.locator("#cpuTerminalPortraitOverlay").getAttribute("src"), /assets\/cpu-portraits\/yuzu-d\.webp$/);
    ...
    await page.locator("#cpuTerminalPortraitOverlay").evaluate((image) => image.dispatchEvent(new Event("error")));
    assert.equal(await page.locator("#cpuTerminalPortraitOverlay").isHidden(), true);
    assert.equal(await page.locator("#cpuTerminalPortraitOverlayFallback").isVisible(), true);
    ...
    await page.waitForFunction(() => document.querySelector("#cpuTerminalCommentarySummary")?.textContent.includes("あなたの投了")
      && document.querySelector("#cpuTerminalPortraitSummary")?.getAttribute("src")?.endsWith("/yuzu-g.webp"));
    await page.locator("#cpuTerminalPortraitSummary").waitFor({ state: "visible", timeout: 5000 });
    assert.match(await page.locator("#cpuTerminalPortraitSummary").getAttribute("src"), /assets\/cpu-portraits\/yuzu-g\.webp$/);
