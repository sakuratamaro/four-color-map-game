...
const EXPECTED_ENGINE_VERSION = "5.0.0-alpha.4";
...
function assertPublicPrivacy(label, publicState) {
  ...
  check(`${label} exposes alpha.4 category window`, categoryWindow?.actor === publicState.active && Array.isArray(categoryWindow.categories));
...
async function run() {
  ...
  activeStage = "initialize alpha.4 room";
  let room = await refreshRoom(session, roomId);
  check("new room uses alpha.4", room.status === "playing"
  ...
  for (let step = 0; step < MAX_DRIVER_STEPS && !(retiredDeclarationChecked && categoryRejectChecked && cpuSteps > 0 && room.publicState?.active === "A"); step += 1) {
    ...
    if (room.publicState.phase === "COLOR" && !retiredDeclarationChecked) {
      ...
      check("alpha.4 no-color declaration is retired", !rejected.ok
        && rejected.status === 400
