"use strict";
// Review039: API-only, one new anonymous profile / Kurogane room / execution.
// Never imports the consumed F3/067 budgets or retries. No browser is operated.
const assert=require("node:assert/strict"),fs=require("node:fs"),path=require("node:path");
const {execFileSync}=require("node:child_process"),{randomUUID}=require("node:crypto");
const {isDeepStrictEqual}=require("node:util");
const CANDIDATE="9590a4212d69185fc93df31b552d9bd870d5a9a3",REVIEW="CHATGPT-REVIEW-20260913-039";
const ORIGIN="https://qkcuhludisairpgzhryl.supabase.co",EDGE="/functions/v1/standard-game-action";
const SNAPSHOT="/rest/v1/rpc/fcg_standard_room_snapshot_v2",POLICY="standard-character-palette-efficiency-v1:kurogane";
...
function classify(endpoint,body,owned){
  ...
  if(body?.operation==="profile"&&body.expectedRevision===0&&body.displayName==="CpuPaletteCanary"&&isDeepStrictEqual(body.profileState,{}))return "read";
  if(body?.operation==="cpu-roster"&&Object.keys(body).length===1)return "read";
  if(body?.operation==="cpu-start"&&!owned&&body.characterId==="kurogane"&&body.confirmed===true&&UUID.test(body.actionId))return "match";
...
function parseSnapshot(raw,owned){
  ...
  return {status:r.status,version:Number(r.version),publicState:r.public_state,
    policyMatches:r.cpu_policy_version===POLICY,characterMatches:r.cpu_character_id==="kurogane",opponentMatches:r.opponent_kind==="cpu"};
}
...
function settlement(actual,before,matchId){
  ...
  return Boolean(matchId&&entries.length===1&&previous.length===0&&entries[0].result==="LOSS"
    &&entries[0].terminalReason==="SURRENDER"&&entries[0].cpuCharacterId==="kurogane"
    &&actual?.profileState?.cpuStats?.losses===before?.profileState?.cpuStats?.losses+1&&actual.revision>before.revision);
}
function errorCode(e){return /^(HTTP_\d{3}|DEADLINE|OPERATION_BUDGET|OWNED_SNAPSHOT_REQUIRED|UNPERMITTED_REQUEST|NO_TOKEN|FRESH_PROFILE_REQUIRED|NEW_KUROGANE_REQUIRED|CURRENT_ROSTER_REQUIRED|ROOM_POLICY_REQUIRED|CPU_NO_PROGRESS|BODY_TOO_LARGE)$/.test(e?.message||"")?e.message:e?.name==="TimeoutError"?"TIMEOUT":"CONTRACT_OR_NETWORK";}
function validateGate(doc){
  ...
  assert.ok(s?.candidate_sha===CANDIDATE&&s.review_id===REVIEW&&s.live_canary_attempts===1
    &&s.live_canary_state==="RESERVED_BEFORE_EXECUTION"&&r?.decision==="APPROVE_WITH_CONDITIONS"
    &&r.subject_sha===CANDIDATE&&r.review_kind==="game_production_release_approval"
    &&r.source?.message_id==="fd9b3fa2-dd74-4379-a0e7-7cf05ad31ff9"&&r.source.response_complete===true&&r.source.kind==="chatgpt"
    &&r.source.thread_id==="6aa229e7-e098-83ee-ac5e-d366a12653a4"
    &&r.source.request_message_id==="718d0bfa-6d26-4931-ac81-22a21542ec50"
    &&r.source.request_body_equality===true,"EXACT_REVIEW_REQUIRED");
  assert.deepEqual(s.live_canary_bounds,{profiles:1,matches:1,character_id:"kurogane",attempts:1,max_seconds:240,cpu_send_attempts:8,surrender_send_attempts:1,additional_matches:0,additional_profiles:0,economy_actions:0,deletions:0,privileged_room_recovery:false});
  ...
  assert.equal(s.base_sha,"d9ce111d7d97019d55b3e90842602001e045ea04");
  assert.equal(s.spec_snapshot_sha,"de0cc9e2299a8a69dd1bfbc9368cd98042ca9b9f");
  assert.equal(s.spec_version,"UDL-051-palette-v1.1");
  assert.equal(s.scope,"Pages_Edge_DB_managed_activation");
  assert.deepEqual(s.db_change_set,["supabase/migrations/202609130002_standard_cpu_palette_efficiency.sql"]);
  assert.deepEqual(s.edge_change_set,["supabase/functions/standard-game-action/index.ts","supabase/functions/standard-game-action/standard-engine.bundle.js"]);
  assert.deepEqual(s.managed_setting_change_set,[{name:"FCG_CPU_PALETTE_EFFICIENCY",compatible_deploy_value:null,
    activation_value:"standard-character-palette-efficiency-v1",
    rollback:"disable new palette activation; retain compatible Edge and additive SQL; preserve existing split setting"}]);
  const ci=s.ci_rerun?.final_evidence;
  assert.ok(s.windows_status==="SUCCESS"&&s.windows_run==="34723536140"&&s.ci_rerun.started_runs===1
    &&ci?.candidate_sha===CANDIDATE&&String(ci.run_id)===s.windows_run&&ci.run_attempt===2
    &&Number.isSafeInteger(ci.job_id)&&ci.job_id>0&&ci.status==="completed"&&ci.conclusion==="success"
    &&ci.read_from_github_api===true,"UNCHANGED_EDGE_RERUN_SUCCESS_REQUIRED");
  const g=s.production_gates;
  assert.ok(g.sql==="APPLIED_5_OF5_VERIFIED"&&g.compatible_edge==="DEPLOYED_BYTE_EXACT_CURRENT_VERIFIED"
    &&g.managed_activation==="PALETTE_VERIFIED_AFTER_460_SECONDS"&&g.main==="EXACT_SHA_PUBLISHED"
...
async function execute({key,fetchImpl=globalThis.fetch,now=Date.now,uuid=randomUUID}){
  ...
  const report={candidate:CANDIDATE,review:REVIEW,mode:"API_ONLY",startedAt:new Date(budget.started).toISOString(),
    ...
    browserUi:"NOT_RUN",physicalDevices:"NOT_RUN",paletteSpecificPosition:"NOT_RUN",kurogane100PrivateHand:"NOT_RUN",winRateImprovement:"NOT_RUN",allTenStockHands:"NOT_RUN",oldWorkerRace:"NOT_RUN",
  ...
  const edge=body=>request(EDGE,body),profile=()=>edge({operation:"profile",expectedRevision:0,displayName:"CpuPaletteCanary",profileState:{}});
  ...
  try{
    ...
    if(!(roster.cpuPolicyGeneration==="palette"&&roster.cpuPolicyCapabilities?.includes("standard-character-palette-efficiency-v1")
      &&roster.characters?.find(x=>x.id==="kurogane")?.policyVersion===POLICY))fail("CURRENT_ROSTER_REQUIRED");
    report.newPolicyRoster="VERIFIED";
    stage="one Kurogane room";const start=await edge({operation:"cpu-start",actionId:uuid(),characterId:"kurogane",confirmed:true});
    if(UUID.test(start.roomId))owned=start.roomId;
    if(!(owned&&start.startStatus==="created"&&start.opponentKind==="cpu"&&start.characterId==="kurogane"))fail("NEW_KUROGANE_REQUIRED");
...
async function run(args){
  ...
  const gov=path.resolve(__dirname,".."),root=path.resolve(gov,"../cpu-palette-efficiency-20260913");
  const doc=JSON.parse(fs.readFileSync(path.join(gov,"docs/CHATGPT_REVIEW_DECISIONS.json"),"utf8"));validateGate(doc);
  assert.equal(fs.existsSync(path.join(gov,"docs/CPU_PALETTE_LIVE_20260913.json")),false,"NO_SECOND_ATTEMPT");
  ...
  const receipt=path.join(gov,"docs/CPU_PALETTE_LIVE_20260913.attempt.json"),fd=fs.openSync(receipt,"wx");
