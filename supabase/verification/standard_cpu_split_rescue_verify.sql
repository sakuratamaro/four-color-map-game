-- Read-only post-migration check for the exact UDL-051-split-v1.1 candidate.
...
with expected_functions(signature, source_md5, definer, language_name, volatility, service_execute) as (
  values
    ('fcg_private.fcg_standard_cpu_policy_is_supported(text,text)', 'e48c9e504fd73d33f9900288fe037334', false, 'sql', 'i', false),
    ('fcg_private.fcg_standard_cpu_policy_is_current(text,text)', 'dc26fae24eb836c26ca5f1e447e92bc6', false, 'sql', 'i', false),
    ('public.fcg_standard_server_start_cpu(uuid,uuid,uuid,text,text,text,jsonb,jsonb,text)', '64063d6e0fdf1033e8bd04e3a65acbf4', true, 'plpgsql', 'v', true)
), identities(id) as (
