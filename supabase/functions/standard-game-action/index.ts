...
function quizPrompt(level: number, recentTemplateIds: string[] = []): QuizGenerated {
  ...
  if (level === 1) catalog = [
    ...
    () => { const lower = 1; const end = secureInt(4, 8); const a = secureInt(1, 4); const b = secureInt(-4, 7); return q("sigma-square", "複合数列", `k=${lower}から${end}まで ${a}k² ${signedTerm(b)} の総和は？`, inclusiveIntegerSum(lower, end, (k) => a * k ** 2 + b), "二乗和：Σk² = n(n+1)(2n+1) ÷ 6", 75, { kind: "sum", index: "k", lower, upper: end, body: `${a}k² ${signedTerm(b)}`, grouped: true, suffix: "= ?" }); },
    ...
    () => { const radius = secureInt(2, 8); const height = secureInt(2, 8) * 3; return q("cone-volume", "体積", `半径${radius}、高さ${height}の円すいの体積は何π？`, radius ** 2 * height / 3, "円すいの体積：V = πr²h ÷ 3", 75, { kind: "geometry", shape: "cone", dimensions: { radius, height } }); },
