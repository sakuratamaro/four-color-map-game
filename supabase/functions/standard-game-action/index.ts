...
const NEW_STANDARD_MATCH_ENGINE_VERSION = "5.0.0-alpha.2";
...
function inclusiveIntegerSum(lower: number, upper: number, term: (index: number) => number): number {
  let total = 0;
  for (let index = lower; index <= upper; index += 1) total += term(index);
  return total;
}
...
function quizPrompt(level: number, recentTemplateIds: string[] = []): QuizGenerated {
  ...
  if (level === 1) catalog = [
    ...
    () => { const base = secureInt(2, 12) * 2; const height = secureInt(2, 12); return q("triangle-area", "面積", `底辺${base}、高さ${height}の三角形の面積は？`, base * height / 2, "三角形の面積：S = 底辺 × 高さ ÷ 2", 38, { kind: "geometry", shape: "triangle", dimensions: { base, height } }); },
    () => { const a = secureInt(2, 8); const b = secureInt(2, 8); const h = secureInt(2, 8); return q("cuboid-volume", "体積", `${a}×${b}の底面で高さ${h}の直方体の体積は？`, a * b * h, "直方体の体積：V = たて × よこ × 高さ", 40, { kind: "geometry", shape: "cuboid", dimensions: { length: a, width: b, height: h } }); },
    ...
    () => { const turtles = secureInt(2, 8); const cranes = secureInt(3, 12); const total = turtles + cranes; const legs = turtles * 4 + cranes * 2; return q("crane-turtle", "鶴亀算", `鶴と亀が合わせて${total}匹、足は${legs}本。亀は何匹？`, turtles, "鶴亀算：全部を鶴と仮定し、足の差を1匹あたりの差で割る", 48, { kind: "story" }); },
    ...
    () => { const lower = secureInt(2, 4); const end = secureInt(8, 12); const a = secureInt(2, 5); const b = secureInt(-7, 7); const c = secureInt(-9, 9); return q("sigma-quadratic", "二次式の総和", `k=${lower}から${end}まで ${a}k² ${signedTerm(b)}k ${signedTerm(c)} の総和は？`, inclusiveIntegerSum(lower, end, (k) => a * k ** 2 + b * k + c), "Σ(ak²+bk+c)=aΣk²+bΣk+cΣ1。下端が1でない分を引く", 60, { kind: "sum", index: "k", lower, upper: end, body: `${a}k² ${signedTerm(b)}k ${signedTerm(c)}`, grouped: true, coefficients: { quadratic: a, linear: b, constant: c }, suffix: "= ?" }); },
    () => { const total = secureInt(9, 13); const selected = secureInt(4, 6); return q("committee-roles", "組合せと役割", `${total}人から${selected}人の委員を選び、その中から委員長と副委員長を1人ずつ決める方法は何通り？`, combination(total, selected) * selected * (selected - 1), "まず委員を選び、その後に委員長・副委員長を順序つきで選ぶ", 58, { kind: "combination", total, selected, suffix: `× ${selected} × ${selected - 1} = ?` }); },
    () => { const x = secureInt(-6, 8); const y = secureInt(-5, 7); const z = secureInt(-4, 9); return q("system-three", "三元連立方程式", `x+y=${x + y}、2y+z=${2 * y + z}、x−z=${x - z} のとき x+y+z は？`, x + y + z, "3式を使って1文字ずつ消去し、最後に3変数を足す", 62, { kind: "system", lines: [`x + y = ${x + y}`, `2y + z = ${2 * y + z}`, `x − z = ${x - z}`], suffix: "x + y + z = ?" }); },
    () => { const p = secureInt(-5, 7); const qv = secureInt(-4, 6); const r = secureInt(-3, 5); const s = secureInt(-6, 8); const t = secureInt(-5, 7); const u = secureInt(-4, 6); const v = secureInt(-3, 5); const w = secureInt(-6, 8); const n = secureInt(-5, 7); return q("determinant-three", "3次行列式", `det([[${p},${qv},${r}],[${s},${t},${u}],[${v},${w},${n}]]) は？`, p * (t * n - u * w) - qv * (s * n - u * v) + r * (s * w - t * v), "1行目で余因子展開し、符号 + − + に注意", 62, { kind: "matrix-determinant", rows: [[p, qv, r], [s, t, u], [v, w, n]], suffix: "= ?" }); },
    () => { const lower = secureInt(-3, 2); const upper = secureInt(4, 7); const a = secureInt(1, 4); const b = secureInt(-5, 5); const c = secureInt(-6, 6); const answer = 2 * a * (upper ** 3 - lower ** 3) + 2 * b * (upper ** 2 - lower ** 2) + 2 * c * (upper - lower); return q("integral-polynomial", "定積分", `${lower}から${upper}まで ${6 * a}x² ${signedTerm(4 * b)}x ${signedTerm(2 * c)} を積分した値は？`, answer, "項別に積分し、上端の値から下端の値を引く", 60, { kind: "integral", lower, upper, body: `${6 * a}x² ${signedTerm(4 * b)}x ${signedTerm(2 * c)}`, variable: "x", suffix: "= ?" }); },
    () => { const radius = secureInt(3, 9); const height = secureInt(3, 8) * 3; return q("cylinder-minus-cone", "複合立体", `半径${radius}、高さ${height}の円柱から、同じ底面・高さの円すいをくり抜く。残る体積は何π？`, radius ** 2 * height * 2 / 3, "円柱 πr²h から円すい πr²h/3 を引く", 55, { kind: "story" }); },
    () => { const a = secureInt(2, 5); const b = secureInt(-6, 7); const c = secureInt(1, 4); const d = secureInt(-5, 8); const x = secureInt(-3, 5); return q("derivative-product", "積の微分", `y=(${a}x ${signedTerm(b)})(${c}x² ${signedTerm(d)}) のとき、x=${x}での dy/dx は？`, a * (c * x ** 2 + d) + (a * x + b) * 2 * c * x, "積の微分：(fg)'=f'g+fg'。最後にxを代入", 60, { kind: "derivative", function: `(${a}x ${signedTerm(b)})(${c}x² ${signedTerm(d)})`, at: x, suffix: "= ?" }); },
    () => { const red = secureInt(5, 9); const blue = secureInt(5, 9); return q("paired-selection", "場合の数", `赤玉${red}個・青玉${blue}個はすべて区別できる。赤2個と青2個を選ぶ方法は何通り？`, combination(red, 2) * combination(blue, 2), "赤の選び方と青の選び方を別々に数えて掛ける", 55, { kind: "story" }); },
    () => { const first = secureInt(-3, 6); const multiplier = secureInt(2, 4); const offset = secureInt(-5, 5); const position = secureInt(5, 7); let answer = first; for (let index = 1; index < position; index += 1) answer = multiplier * answer + offset; return q("recurrence", "漸化式", `a₁=${first}、aₙ₊₁=${multiplier}aₙ ${signedTerm(offset)}。a${position} は？`, answer, "前の項を代入して、符号に注意しながら順に計算", 52, { kind: "story" }); },
  ];
...
function quizExperienceMeta(level: number, question: QuizGenerated): JsonObject {
  ...
  if (kind === "story") mission = `条件を整理して、${question.category}の答えを求めよう`;
    ...
    : question.templateId === "matrix-trace"
      ? "行列積の対角成分を求め、tr(AB)を計算しよう"
      : "行と列を組み合わせ、指定された成分を求めよう";
  else if (kind === "system") mission = question.templateId === "system-three"
    ? "3つの式から x+y+z を求めよう"
    : "一方の文字を消去して、x を求めよう";
