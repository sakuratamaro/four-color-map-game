-- UDL-051 F1: add palette-efficiency policy; preserve all21 saved versions.
...
as $$
  select coalesce(case
    when p_character_id = 'kurogane' then p_policy_version in (
      ...
      'standard-character-palette-efficiency-v1:kurogane'
    )
    when p_character_id in ('yuzu','ren','minato','koharu','aoi','kai','tsubasa','shion','rei')
      then p_policy_version in (
        ...
        'standard-character-palette-efficiency-v1:' || p_character_id
      )
    else false
  end, false);
$$;
