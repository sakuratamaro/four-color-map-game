...
| P1 | 0マス時の全開始候補表示 | UX＋a11y＋privacy＋司令塔 | LOCAL_VERIFIED | 単独`89f1243`、統合`8ec96c4`。0マス時は既存の公開盤面geometryと選択完了判定だけから、必要数まで完成できる開始候補を水色破線ですべて表示する。選択数0、自動選択0、旧単一おすすめdataset撤去、pointer／keyboard、390×844、対象skill、turn切替／reload、最大12×12、action送信0を固定。後続接触feedbackまで含む統合static＋台帳83/83、実Chrome/Edge各6/6。main／Pages未統合 |
| P1 | 各セル選択直後の接触フィードバック | UX＋a11y＋privacy＋司令塔 | LOCAL_VERIFIED | 単独`2235468`、統合`cc6f2b5`。本人のlocal draftだけを選択・解除直後に再判定し、色数が上がった時だけ2／3／4色の段階を即時表示する。required-size完成を待たず、同色数の追加は再演0、解除で古い表示を消し、公開trace／pollは表示0。pointer＋keyboard、390px、reduced-motion、action送信0を固定。統合static＋台帳83/83、先行5変更を含む実Chrome/Edge各6/6。main／Pages未統合 |
...
| UDL-20260909-035 | 持ち色の初期重複／消失と変更通知 | 初期基本色とおまけ色を重複させず、後発の持ち色変更は当事者へ行為者、skill、対象枠、変更前後色を通知・履歴化する。同色化しても基本／おまけ枠を統合せず、初期値と現在値を区別する | server-authoritativeな初期配布、基本2枠＋おまけ枠、残数、reload、効果終了、相手private非漏えい、通知一回性を確認 | UDL-019、UDL-028、palette impact event | ルール＋Edge＋UX＋privacy | Palette semantics follow-up | LOCAL_VERIFIED | `1a1d8f8`、統合`9f57c12` | NO | NO | 単独関連unit/static 194/194・実Chrome/Edge各1/1、累積統合source/static/docs 211/211・対象実Chrome/Edge各7/7。初期3色別配布の保存、現在値との分離、同色slot維持、12件private履歴、actor/skill/slot/前後色、旧room互換、reload一回性、相手projection非漏えいと先行6変更の非回帰を確認 | 2026-09-09追加監査 | `4cefe9f`はtorn snapshot拒否だけを解決。新候補はEdge bundle変更を含むため、前段の小分け公開後にEdge→canary→Pagesで公開する | YES |
...
| UDL-20260909-039 | 0マス時に左上1候補だけでなく実際に選べる全候補を見たい | 0マス時は既存ルールで実際に選択可能な開始候補をすべて案内強調し、選択数は0のままにする | 自動選択0、focus ringと区別、全候補過不足0、案内のための新制約0、pointer／keyboard、390px | board-first guidance、server geometry | UX＋a11y＋privacy | All start candidates | LOCAL_VERIFIED | `89f1243`、統合`8ec96c4` | NO | NO | 最新統合Chrome/Edge各6/6、390px exact候補5件、最大12×12、action送信0 | 2026-09-09追加監査 | 公開の単一おすすめ候補を置換。統合static＋台帳83/83、対象skill／turn切替／reload、後続接触feedbackまでローカル確認 | YES |
| UDL-20260909-040 | 2／3／4色接触演出を1マス選択ごとに即時反応させたい | 自分のlocal draftだけを各選択・解除直後に再判定し、2→3→4色の閾値上昇時だけ発火する | required-size／送信待ちにしない、同色数連打0、解除追随、CPU／相手／poll／reload／replay派手演出0、常時outline 0、reduced-motion | UDL-018、public board geometry | UX＋a11y＋privacy | Per-cell contact feedback | LOCAL_VERIFIED | `2235468`、統合`cc6f2b5` | NO | NO | 最新統合Chrome/Edge各6/6、390px、reduced-motion、action送信0 | 2026-09-09追加監査 | 完成時だけの判定を各local選択差分へ置換。統合static＋台帳83/83、公開trace／poll再演0と先行5変更の累積を確認 | YES |
| UDL-20260909-041 | 「塗れる色が見つからないとき」の長文案内を削除したい | 長文説明を撤去し、通常の色、既存skill、投了操作だけを残す | 自動敗北、合法色oracle、時間制限を復活させず、救済skillと投了のserver境界、keyboard、390pxを維持 | UDL-006、common COLOR response | UX＋ルール＋a11y | COLOR guidance trim | LOCAL_VERIFIED | `608962e` | NO | NO | 静的85/85、390px online Chrome/Edge各1/1、Local Chrome/Edge各3/3。通常色、色操作skill導線、投了、reload、保存境界を維持し、長文・旧申告・合法色oracle 0 | 2026-09-09追加監査 | B便→報酬便後続のPages-only候補。Windows gate・公開実見までREOPENED | YES |
| UDL-20260909-042 | 完了報酬が獲得表示だけで所持券へ永続化されない報告 | 保存成功後だけ獲得表示し、付与前後、ガチャ画面、reload後の所持数を一致させ、結果再表示／再戦でも二重付与しない | matchId冪等、CPU／PvP、勝敗、未精算、応答喪失、reload、同時tab、券消費との整合 | settlement receipt、profile tickets、UDL-046 | 進行＋Edge＋DB＋UX | Reward persistence P0 | LOCAL_VERIFIED | `65f23c9`（canary `eb629e5`） | NO | NO | 本番backend 18/18、関連113/113、390px focused Chrome/Edge各1/1。配備済みEdge index/bundleは候補とbyte/SHA一致。CPU敗北の付与、profile delta、完全reload、同一終局action再送の非二重付与を実測 | 2026-09-09追加監査 | backend不具合は再現せず、UIを保存済み総数に拘束して`所持 2→3`と消費後総数を明示。B便公開後にPages-onlyで公開実見するまでREOPENED | YES |
...
| 初期palette重複、持ち色変更通知・履歴、基本／おまけ枠分離 | LOCAL_VERIFIED | `1a1d8f8`、統合`9f57c12`で初期値保存、12件private履歴、actor/skill/slot/前後色、旧room fallbackを実装。累積統合211/211と実Chrome/Edge各7/7を通過。main・Edge・Pagesは未反映 |
...
| 0マス時の実選択可能な全候補 | LOCAL_VERIFIED | 単独`89f1243`、統合`8ec96c4`で全候補を過不足なく表示し、選択数0、自動選択0、action送信0をChrome/Edgeで確認。main／Pagesは未統合。`UDL-039` |
| 2／3／4色接触を各セル選択直後に再判定 | LOCAL_VERIFIED | 単独`2235468`、統合`cc6f2b5`でrequired-size完成時だけの判定を置換。閾値上昇だけ即時発火し、同色数追加0、解除追随、公開trace／poll再演0をChrome/Edgeで確認。main／Pagesは未統合。`UDL-040` |
| COLOR長文案内撤去 | LOCAL_VERIFIED / REOPENED | `608962e`でOnline／Localの長文を撤去し、通常色・色操作skill・投了を維持。B便→報酬便後続でWindows gate、Pages公開実見待ち。`UDL-041` |
| 完了報酬の実所持・reload永続化 | LOCAL_VERIFIED / REOPENED | `65f23c9`で保存済み総数に獲得表示を拘束し、終局・ガチャ・reloadの数を明示。backend 18/18と両browser合格。B便後続の独立Pages公開・公開実見待ち |
