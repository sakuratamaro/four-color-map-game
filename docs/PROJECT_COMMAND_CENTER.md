...
| P1 | CPU封印skillの局面判断 | CPU＋privacy＋ルール | LOCAL_VERIFIED | 候補branch `codex/cpu-seal-timing-release-20260910`。公開盤面上の直後の応答候補が3色以上なら温存し、2色以下からさらに減らせる時だけ高評価。turn一律禁止なし、相手private参照なし。局面／roster／bundle 43/43、formal self-play 10/10、実browser CPU 1/1、両bundle二巡SHA不変。Windows、Edge source readback、live CPU canary、main／Pagesは未実施 |
...
| UDL-20260909-045 | CPUが封印skillを弱い局面で浪費する | 序盤一律禁止にせず、接触色が少なく代替手段が多い時は温存し、応手を絞れる局面では初手を含め使用できる判断へする | 公開／許可情報だけ、相手private palette／hand参照0、全CPU合法・決定性、弱体化0、seeded局面比較 | CPU policy、public contact colors、seal skills | CPU＋privacy＋ルール | CPU seal timing | LOCAL_VERIFIED | branch `codex/cpu-seal-timing-release-20260910` | NO | NO | 局面／roster／bundle 43/43、formal self-play 10/10、実browser CPU 1/1。1接触ではfavorite封印も温存、2接触ではturn 1でも公開応答2→1の対象色を選びauthoritative accepted。opponent private noiseで選択不変。Edge／Local bundle二巡SHA不変 | 2026-09-09追加監査 | random封印は結果を確定扱いせずpotentialとして採点。DB、migration、RPC、rule、報酬変更なし。Windows、Edge配備、live canary、main／Pages待ち | YES |
...
| CPU封印skillの局面判断 | LOCAL_VERIFIED | 公開応答候補3色以上は温存、2色以下から減らせる時だけ高評価。turn 1、対象色／空振り色、privacy、決定性、authoritative acceptedを確認。`UDL-045` |
