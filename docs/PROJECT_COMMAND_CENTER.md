...
| P1 | Shift対象指定と説明の摩擦解消 | UX＋ルール＋司令塔 | LOCAL_VERIFIED | `041ed22`。行・列を選んで盤面tap/keyboardで対象帯、方向だけ左/右・上/下にした。Half全帯、Triple外周理由付き拒否、黄中央＋紫隣接、取消無送信、zero-based payload、390/768/1365pxを独立監査GO。select候補`24caae8`は不採用のまま。音便後にv39/v38へrebaseして次便公開 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| UDL-20260906-001 | Shiftを角膨張と同じ盤面操作へ | 行・列・中央帯を盤面tap/keyboard、方向だけ自然操作 | 390px/PC、focus、取消、再送、no-oracle、zero-based payload維持 | UDL-014、Half/Triple現行rules | UX＋ルール＋司令塔 | Shift board UX | LOCAL_VERIFIED | `041ed22`（旧`24caae8`は不採用） | NO | NO | Edge/Chrome・390/768/1365合格 | `01a0762c` | select方式をSUPERSEDED、ユーザー要望不一致 | YES |
