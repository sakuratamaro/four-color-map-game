...
| P1 | Shift対象指定と説明の摩擦解消 | UX＋ルール＋司令塔 | MERGED | 製品`ad49a41`、main `4b2ea3d`。行・列を選んで盤面tap/keyboardで対象帯、方向だけ左/右・上/下にした。Half全帯、Triple外周理由付き拒否、黄中央＋紫隣接、取消無送信、zero-based payloadを維持。独立Chrome/Edge各3/3・skip 0、Windows gate `34041850645`成功。Pages反映待ち |
...
| P1 | 同一ターン同一カテゴリ1枚制約 | ルール＋CPU＋UX＋Edge | SPEC_READY | `STANDARD_SKILL_CATEGORY_AUDIT.md`で19枚を5 color／6 area／8 disruptに固定し、別`usageCategory`、同一操作権区間、accepted no-op、retry/replay、CPU/debug/LAB、alpha.3境界を契約化。次はengine・CPU・client/server bundleを原子的に実装する |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| UDL-20260906-001 | Shiftを角膨張と同じ盤面操作へ | 行・列・中央帯を盤面tap/keyboard、方向だけ自然操作 | 390px/PC、focus、取消、再送、no-oracle、zero-based payload維持 | UDL-014、Half/Triple現行rules | UX＋ルール＋司令塔 | Shift board UX | MERGED | `ad49a41`（旧`24caae8`は不採用） | `4b2ea3d` | PENDING | Windows `34041850645` Chrome/Edge成功、独立各3/3・skip 0 | `01a0762c` | select方式をSUPERSEDED、ユーザー要望不一致 | YES |
