...
| P1 | 0マス時の全開始候補表示 | UX＋a11y＋privacy＋司令塔 | LOCAL_VERIFIED | 単独`89f1243`、統合`8ec96c4`。0マス時は既存の公開盤面geometryと選択完了判定だけから、必要数まで完成できる開始候補を水色破線ですべて表示する。選択数0、自動選択0、旧単一おすすめdataset撤去、pointer／keyboard、390×844、対象skill、turn切替／reload、最大12×12、action送信0を固定。統合static＋台帳82/82、報酬／COLOR案内／封印色を含む実Chrome/Edge各4/4。main／Pages未統合 |
| P1 | 各セル選択直後の接触フィードバック | UX＋a11y＋privacy＋司令塔 | LOCAL_VERIFIED | `2235468`。本人のlocal draftだけを選択・解除直後に再判定し、色数が上がった時だけ2／3／4色の段階を即時表示する。required-size完成を待たず、同色数の追加は再演0、解除で古い表示を消し、公開trace／pollは表示0。pointer＋keyboard、390px、reduced-motion、action送信0を固定。focused static 75/75、実Chrome/Edge各2/2。main／Pages未統合 |
...
| UDL-20260909-039 | 0マス時に左上1候補だけでなく実際に選べる全候補を見たい | 0マス時は既存ルールで実際に選択可能な開始候補をすべて案内強調し、選択数は0のままにする | 自動選択0、focus ringと区別、全候補過不足0、案内のための新制約0、pointer／keyboard、390px | board-first guidance、server geometry | UX＋a11y＋privacy | All start candidates | LOCAL_VERIFIED | `89f1243`、統合`8ec96c4` | NO | NO | 統合Chrome/Edge各4/4、390px exact候補5件、最大12×12、action送信0 | 2026-09-09追加監査 | 公開の単一おすすめ候補を置換。統合static＋台帳82/82、対象skill／turn切替／reload、報酬／COLOR案内／封印色の累積をローカル確認 | YES |
| UDL-20260909-040 | 2／3／4色接触演出を1マス選択ごとに即時反応させたい | 自分のlocal draftだけを各選択・解除直後に再判定し、2→3→4色の閾値上昇時だけ発火する | required-size／送信待ちにしない、同色数連打0、解除追随、CPU／相手／poll／reload／replay派手演出0、常時outline 0、reduced-motion | UDL-018、public board geometry | UX＋a11y＋privacy | Per-cell contact feedback | LOCAL_VERIFIED | `2235468`、統合`cc6f2b5` | NO | NO | Chrome/Edge各2/2、390px、reduced-motion、action送信0 | 2026-09-09追加監査 | 完成時だけの判定を各local選択差分へ置換。focused static 75/75、公開trace／poll再演0を確認 | YES |
