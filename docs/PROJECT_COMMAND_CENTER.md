...
| UDL-20260910-050 | 仕様・要望台帳・テストを両AIの共有正本として参照し、使用量とロールバック事故を減らす | 新しい司令塔を作らず、既存UDL・spec・tests・release evidenceへ単一入口を設ける。REQ importはcrosswalkに限定し、意図／実装／検証／公開を別軸で記録する。起動receipt、単一owner、ChatGPT reviewの対象SHA固定、無応答非承認を導入する | 正本path／version／base SHAが明示され、28 REQ aliasの出典・UDL対応・test・証拠を追跡でき、未対応や未検証は明示する。代表contract testがPASS。指示受領→台帳反映→結果報告→同一ChatGPT review→判定保存を非本番で1巡し、手動区間を報告する | UDL-016、UDL-017、既存司令塔・連携、worktree inventory | 司令塔 | Shared canon v1.1 | IMPLEMENTING | — | NO | NO | NOT_RUN | 2026-09-10 current user goal + reconciled ZIP draft | ゲーム機能、main、Pages、Edge、DBを変更しない文書導入。文書承認をゲーム公開承認へ流用しない | YES |
