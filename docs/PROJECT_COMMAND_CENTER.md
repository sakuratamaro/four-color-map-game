...
| UDL-20260910-055 | 終了した試合へ戻ると開始時のランダム要素告知が再表示される | 終局・復元中を表示済み記録より優先して開始告知を抑止し、同じ終了結果を復元する | cold reload・新規tab・背景復帰で一瞬の表示も0。既存timer解除、結果・戦績・券・room保持、再抽選・二重精算0。新対局・明示再戦の新matchIdは通常告知 | UDL-042、authoritative room/public state、sessionStorage | 既存Codex司令塔（単独割当） | Terminal setup notice Pages-only | IMPLEMENTING | — | NO | NO | Chrome/Edge focused各5/5、全体gate実行中。詳細TERMINAL_SETUP_NOTICE_FIX_20260910.md | ChatGPT user bbb21a1e-346f-438d-8f41-2ecdcb75bf37、同一ChatGPT受入927e69f3-5491-4ab3-9af8-4476a4dc1fb8、このCodexで2026-09-10本番反映指示 | 終局優先がなかった表示不具合を回帰として追跡。DB・Edge・ルールは不変。古いAPPROVE_DOCSは公開承認へ流用しない | YES（ユーザーの本番反映指示） |
