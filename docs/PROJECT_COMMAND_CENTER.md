...
| P0 | 基本paletteの緑が一時消失して復帰した経路の監査 | UX＋同期＋privacy＋司令塔 | DECIDED | 原因を決め打ちせず、authoritative data、current-seat private projection、render lifecycleを同一時系列で照合する。基本色の緑が明示的な効果なしに消失・復帰しないこと、相手paletteを漏らさないことを再現試験と回帰試験で証明する。製品修正は監査後に別便で決める |
