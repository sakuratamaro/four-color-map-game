...
| UDL-20260910-049 | コイン不足でも見た目購入buttonが押せ、汎用エラーになる | 未所持の有料見た目は残高不足なら`あとNコイン`を表示してdisabledにし、カード売却など保存済みprofile更新後は残高と購入可否を即時同期する。server競合の`INSUFFICIENT_COINS`はprivate情報を出さない日本語へ正規化する | 0／境界直前／同額／購入済み／トロフィー解除、pointer／Enter／Space、quote通信0、売却確定後だけ有効化、reload、390px、既存購入・取消・retryを確認 | card-sale profile hydration、cosmetic projection、PUBLIC_FUNCTION_ERRORS | UX＋進行＋a11y＋privacy | Cosmetics affordability | LOCAL_VERIFIED | `f26f78a` | NO | NO | client/static 122/122、対象の売却・購入・不足導線Chrome/Edge各3/3。不足時`あと10コイン`＋disabled＋quote 0、保存済み売却590→600後に即時有効化 | 2026-09-10追加監査 | DB、Edge、価格、所持判定、receiptを変更せずPages-only。Windows gate・公開後確認前はPUBLIC_VERIFIEDにしない | YES |
