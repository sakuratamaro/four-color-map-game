...
| UDL-20260909-035 | 持ち色の初期重複／消失と変更通知 | 初期基本色とおまけ色を重複させず、後発の持ち色変更は当事者へ行為者、skill、対象枠、変更前後色を通知・履歴化する。同色化しても基本／おまけ枠を統合せず、初期値と現在値を区別する | server-authoritativeな初期配布、基本2枠＋おまけ枠、残数、reload、効果終了、相手private非漏えい、通知一回性を確認 | UDL-019、UDL-028、palette impact event | ルール＋Edge＋UX＋privacy | Palette semantics follow-up | LOCAL_VERIFIED | `1a1d8f8`、統合`9f57c12` | NO | NO | 関連unit/static 194/194、実Chrome/Edge各1/1。初期3色別配布の保存、現在値との分離、同色slot維持、12件private履歴、actor/skill/slot/前後色、旧room互換、reload一回性、相手projection非漏えいを確認 | 2026-09-09追加監査 | `4cefe9f`はtorn snapshot拒否だけを解決。新候補はEdge bundle変更を含むため、前段の小分け公開後にEdge→canary→Pagesで公開する | YES |
...
| 初期palette重複、持ち色変更通知・履歴、基本／おまけ枠分離 | LOCAL_VERIFIED | `1a1d8f8`、統合`9f57c12`で初期値保存、12件private履歴、actor/skill/slot/前後色、旧room fallbackを実装。main・Edge・Pagesは未反映 |
