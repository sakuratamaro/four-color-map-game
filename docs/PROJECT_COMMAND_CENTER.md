...
| P1 | 0マス時の全開始候補表示 | UX＋a11y＋privacy＋司令塔 | LOCAL_VERIFIED | `89f1243`。0マス時は既存の公開盤面geometryと選択完了判定だけから、必要数まで完成できる開始候補を水色破線ですべて表示する。選択数0、自動選択0、旧単一おすすめdataset撤去、pointer／keyboard、390×844、対象skill、turn切替／reload、最大12×12、action送信0を固定。focused static 74/74、実Chrome/Edge各1/1。main／Pages未統合 |
...
| UDL-20260909-039 | 0マス時に左上1候補だけでなく実際に選べる全候補を見たい | 0マス時は既存ルールで実際に選択可能な開始候補をすべて案内強調し、選択数は0のままにする | 自動選択0、focus ringと区別、全候補過不足0、案内のための新制約0、pointer／keyboard、390px | board-first guidance、server geometry | UX＋a11y＋privacy | All start candidates | LOCAL_VERIFIED | `89f1243` | NO | NO | Chrome/Edge各1/1、390px exact候補5件、最大12×12、action送信0 | 2026-09-09追加監査 | 公開の単一おすすめ候補を置換。focused static 74/74、対象skill／turn切替／reloadをローカル確認 | YES |
...
| 0マス時の実選択可能な全候補 | LOCAL_VERIFIED | `89f1243`で全候補を過不足なく表示し、選択数0、自動選択0、action送信0をChrome/Edgeで確認。main／Pagesは未統合。`UDL-039` |
