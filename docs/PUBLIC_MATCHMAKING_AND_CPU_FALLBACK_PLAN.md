...
状態: 人間同士の野良対戦、同意制CPU、即時CPU、10人ロスター、CPU再戦、クロガネv2を本番DB・Edge・Pagesへ適用済み。自動live canaryとWindows Chrome/Edgeは合格。別々の物理二端末による最終受入だけは未実施。
...
クロガネは新規roomだけ`standard-character-roster-v1:kurogane-lookahead-v2`を使い、相手の公開接触色と公開封印から、渡した領域で相手が選べる色数を先読みする。人間のprivate palette/loadoutは参照しない。旧v1 roomはそのまま再生し、成功した再戦でだけv2へ更新する。
...
- [PUBLIC_VERIFIED] 非公開チケット、検索receipt、heartbeat TTL、取消、レート制限を含むmigrationと公開UI。
- [PUBLIC_VERIFIED] 確定後は合言葉を表示せず、既存の6枚セット・対戦同期へ引き継ぐ。
- [PUBLIC_VERIFIED] Runbook C 210/210で16 profile、2 finder、取消/検索競合、10同時claim、完走、再検索、秘密非公開を確認。
- [NOT_RUN] 別々の物理二端末での野良対戦完走、片側再読込、新しい試合。

### 第2段階: 同意制CPUフォールバック

- [PUBLIC_VERIFIED] 90秒/180秒案内、即時CPU選択、明示同意、チケット行ロック、ログイン不能CPUプロフィールを本番適用済み。
- [PUBLIC_VERIFIED] 10人のversion付きロスター、固定6枚、性格、台詞、得意/苦手/お気に入り表示、決定的action IDを公開済み。
- [PUBLIC_VERIFIED] 対人/CPU別集計、キャラクター別戦績、同じCPUとの再戦、別CPUとの新試合を自動live canaryとWindows browserで確認。
- [PUBLIC_VERIFIED] Runbook D 107/107と即時CPU完走25/25、新クロガネv2 canaryで合法手、決着、再戦を確認。
- [NOT_RUN] 物理端末でのCPU勝利、報酬/戦績の再読込、クロガネの体感難易度。
