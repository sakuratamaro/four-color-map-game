...
状態: プロダクト・実装構想のみ。この文書の追加では、公開版・データベース・Edge Functionは変更しない。
...
- CPUを選んだ場合は、`ミドリ隊員` のようなキャラクター名で通常の対戦開始演出を行い、小さく常時 `CPU` と表示する。
...
- 各カードには顔、名前、短い一言、得意、苦手、お気に入りスキルを表示する。
- 強さの星・レベル・Easy/Hardは表示しない。
- 最初は最近選んだ3人を上に出しつつ、`全員を見る` で必ず10人から選べる。
...
- 基本CPUで最後まで完走できるまでは、スキルカードを使わせない。
...
- 追加的なチケットmigrationとサーバー操作。
- ロビー操作と再読込復帰。
- 既存の6枚セット・対戦導線を変更せず再利用。
- CPUはまだ入れない。

### 第2段階: 同意制CPUフォールバック

- 90秒・180秒の案内。
- 人間参加とCPU承諾が競合した際の原子的な決着。
- 予約CPUメンバー・プロフィールとサーバー側思考ロジック。
- 10人分の固定ロスター、6枚ロードアウト候補、思考パラメーター、固定台詞。
- キャラクター選択とキャラクター別戦績。
- 対人戦とCPU戦を分けた成績処理。
