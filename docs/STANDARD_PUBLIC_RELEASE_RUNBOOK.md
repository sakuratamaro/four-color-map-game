...
## Local CPU progression UI checkpoint — not a release authorization

UDL011/064 design-v2's pilot continues locally on the UI172 lineage. Current UI markers: `app.js?v=20260914-10`, `style.css?v=20260914-4`, `standard-online-client.js?v=20260914-1`, `result-continuation.js?v=20260914-2`, `cpu-progression-model.js?v=20260914-1`. The existing preflight checks the new model's exact served bytes. The cumulative pilot requires unapplied migrations 202609140001/002/003, compatible Edge code/bundle and a separately approved exact managed activation; the production flag remains unchanged/default OFF. These local markers and tests do not establish publication or reuse an earlier Pages-only approval. See `CPU_PROGRESSION_UI_CHECKPOINT_20260914.md` for implemented, tested and remaining acceptance. Historical release sections below retain their original evidence.

更新日: 2026-09-10

## CPU progression pilot — complete candidate rollout plan (unpublished)

CI-only follow-up: run [34857220680](https://github.com/sakuratamaro/four-color-map-game/actions/runs/34857220680) on exact `e257841e238f32c9cb40bdcc1c0a8e6c398e1282` completed Chrome contracts 835/835, real PostgreSQL 11/11 with zero skips, and online 197/197. Edge completed contracts 835/835 and lifecycle 79/79, then reached the workflow's 20-minute job cap while still progressing through the online suite. GitHub's job annotation explicitly reports that cap; the partial online summary is 175 passes, zero assertion failures and one cancelled file envelope (176 reported, not the required 197 completed). This candidate changes only the overall job cap to a finite 30 minutes and its static assertion; the matrix, dependency pins, generated checks, complete test lists, concurrency=1 and every individual test/action/teardown timeout remain unchanged. No game, SQL, bundle, UI or rule bytes change. The implementation-byte equivalence below applies to the earlier implementation checkpoint; the workflow/static guard described here are the additional CI-only differences. Require a new exact-candidate review and an independently authorized finite validation before dispatch. Do not rerun the closed older run or renew its spent observation budget, and do not count Chrome-only success as the full gate.

The source candidate is the fixed commit carrying this plan on `codex/cpu-progression-release-20260914`; record its full SHA in the review receipt. Its game and generated artifacts remain byte-identical to the local implementation checkpoint `08ebed0bd1c08f7a2a96c9ae1e112b5e9ed5beba`; the previously validated tests are retained except for the workflow-cap static assertion described above, and public technical documentation is integrated separately. The first scoped candidate's Windows failure (missing technique-state module in the CPU browser fixture and inconsistent timeout-declaration formatting) is preserved; only those test defects were corrected, without changing game behavior or weakening assertions. The subsequent fixed candidate b37e033 retained a second Windows failure: both online suites passed 196/197, with the same legacy DOM-order expectation omitting the separate technique-controls section. That failure was reproduced locally; this checkpoint includes the approved section in the exact order and adds an explicit hidden-state assertion for ordinary matches. The focused four-scenario test passes in Chrome and Edge; game, SQL and generated bundles are unchanged. The original CI clock and all three spent checks remain closed, and this local repair is not new hosted Windows success. Fresh GitHub main was read as `70e691b6f8f1d808476e80990d20df7862bfb782` on 2026-09-14. The full review/export diff is **that main SHA to the candidate**, not only the pilot subrange starting at UI172. It includes already-integrated unpublished public-match, surrender face, terminal, quiz, gacha B1, Home, profile and card-recovery changes. Do not call their older SHA-specific reviews an approval of this combined candidate. A combined release decision must expressly cover those parent changes and replace their serial publication preconditions; otherwise their outstanding gates remain. The active exact-SHA review, own Windows run and publication receipt are authoritative; prose in this plan is not approval.

The existing `docs/STANDARD_MODE_SPEC.md` contains the public technical pilot rules and executable acceptance mapping. Fourteen private provenance/checkpoint documents remain in the preserved local implementation/governance history and are not exported by this branch. Historical references to those checkpoint filenames below identify local evidence, not publicly available files or release approval. No conversation bodies, private message IDs, images, ZIPs or credential files are added by this scoped export. Own fixed-candidate tests and genuine review remain necessary even when implementation bytes match the earlier checkpoint.

Design: `UDL011064-pilot-design-v2`, section SHA256 `d81198558f0e20b4891f1d6082239c42b8dd7815c80ca5ad0f699eb24109a285`. Preserve all ten existing CPU identities and ordinary policy behavior, the separately adopted UI changes, six-card loadouts, and alpha1–alpha4 compatibility. No artwork is added. This plan does not import held F1/F2 code, palette-efficiency migration 202609130002, or governance conversation exports.

Exact new DB set, in this order (three files only):

1. `supabase/migrations/202609140001_standard_learned_technique_profiles.sql`
2. `supabase/migrations/202609140002_standard_ren_trial_template.sql`
3. `supabase/migrations/202609140003_standard_cpu_trial_runtime.sql`

The effective baseline includes the published F3 `202609130001_standard_cpu_split_rescue.sql`; a fresh function/ACL comparison must confirm it before applying anything. Migration-history rows have a known manual-history gap and are not sufficient proof alone. Do not replay earlier migrations to fill that gap. Apply the reviewed three-file set in one transaction, after a read-only precondition/schema/ACL check; rollback the transaction on failure. Read back the created tables, constraints, functions, triggers and service-only ACLs. Do not edit existing room/profile rows or drop schema to recover.

Edge set: `supabase/functions/standard-game-action/index.ts` and its generated `standard-engine.bundle.js`, together. Preserve JWT verification and existing managed values. In particular, retain the published `FCG_CPU_SPLIT_RESCUE=standard-character-split-rescue-v1`; do not reset normal CPU strength while introducing the trial. The only newly proposed managed setting is `FCG_CPU_PROGRESSION_PILOT`, absent/OFF during compatibility deployment, then exact value `ren-unseal-v1` after the activation gates. Absence is not the literal string `null`.

| Stage | Required operation and evidence | Failure boundary |
| --- | --- | --- |
| 0 | Freeze complete SHA, own Windows success and genuine candidate-specific Astra release decision, including DB/Edge/managed-setting set and bounded live scope. Fresh main/Pages/Edge/SQL/setting baseline and existing source readback. | No production change without these gates. Standing UDL056 means no extra routine user-permission wait after they pass. |
| 1 | Deploy the exact compatible Edge pair while pilot creation stays OFF; verify both files byte-for-byte and JWT setting. Existing ordinary CPU remains on its saved/current split policy. | With no alpha5 creation, keep pilot OFF; restore only the exact recorded prior compatible source if necessary. |
| 2 | Apply only the reviewed additive DB transaction and read back schema/function/ACL invariants. | Transaction failure leaves prior DB intact; do not DROP or rewrite user progress. |
| 3 | Publish the reviewed Pages/local assets; verify the Pages SHA/run, exact assets and fresh UI with the feature still OFF. | Restore recorded Pages baseline while keeping compatible Edge and additive DB. |
| 4 | After confirmed compatible Edge completion/readback, preserve a conservative 460-second worker-drain margin and verify the applicable platform limit plus supported capability again. Enable only the exact reviewed pilot value. | Elapsed time alone does not prove global propagation. If source/capability/limit is uncertain, retain OFF. |
| 5 | Run the separately bounded approved live plan and confirm trial/player technique behavior, persistence and ordinary-policy retention. Record actual outcomes and any NOT_RUN steps. | Stop new starts on regression, keep compatible Edge/DB/assets sufficient for existing alpha5 rooms. No silent retries, data injection or extra profiles/matches. |

Supabase currently documents worker lifetimes of 150 seconds on Free and 400 on paid plans, not a guarantee of worldwide deployment propagation. The 460-second interval is a conservative operational prerequisite, not a simulated test or global convergence certificate. See [runtime limits](https://supabase.com/docs/guides/functions/limits). Managed values normally become available without a code redeployment; verify the actual setting/capability and use only the same reviewed compatible artifacts if the environment requires redeployment. See [environment variables](https://supabase.com/docs/guides/functions/secrets).

Stopping the pilot disables new trial/equip requests and new learned-technique creation; keep the compatible worker that can load, act on, settle and recover existing alpha5 snapshots. Existing in-match technique controls read the snapshot, not roster activation. Recover an existing room through the owner-scoped active-room/snapshot path even while a new trial request is disabled. Keep the permanent learned/equipped/clear records. Never reinstall the pre-pilot worker once an alpha5 room can exist: the rehearsal explicitly proves that old worker rejects that state. Do not downgrade stored room engine/policy, refill a used technique, reset ownership or delete rooms. Re-enabling the exact managed value renews the single use only on the next eligible new match. Existing old rooms retain their original snapshot and use count.

Local evidence: `tests/standard-cpu-progression-rollout.test.cjs` exercises the exact baseline source, new worker before pilot SQL, additive migration with active rooms, old/new ordinary CPU progression afterward, trial WIN with new starts disabled, preservation of learned ownership/equipment, and OFF/new-alpha4 then ON/new-alpha5. `standard-cpu-progression-browser.test.cjs` covers native UI/actual worker/SQL use and reload; `standard-cpu-progression-postgres.test.cjs` separately proves real lock races. All use isolated platform fixtures and cannot establish gateway auth, secret propagation, production deployment or physical devices. Preserve original failures and the earlier legacy-policy test boundary.

Before requesting release review, attach a finite live plan using legitimately saved Ren wins. Creating a new test profile does not grant that prerequisite. No production SQL seeding of wins, forced winner, client authority bypass, or reuse of an expired prior smoke allowance. If the permitted ordinary game does not earn a win, record trial gameplay NOT_RUN instead of manufacturing eligibility or repeatedly creating games. A trial unlock badge or asset-byte match alone is not full live gameplay acceptance. The actual permitted profiles, matches, actions, deadline and cleanup/surrender behavior must be bound to the new candidate review and then recorded before execution.

Proposed finite live scope for that review (not yet approved or executed): use one fresh isolated Chrome context and at most one newly created test-player profile, never the user's existing profile. Through the normal game UI/API, allow at most one ordinary Ren game to earn the prerequisite; only on its committed legitimate WIN allow one Ren trial; only on its committed trial WIN allow one equipment operation and one new ordinary Ren game to inspect/use the learned technique. The CPU-owned records created by those normal starts are incidental parts of the same maximum three games, not extra manually created profiles. Use normal six-card inventory and do not buy, draw, sell, answer quizzes, alter RNG, seed records, expose hidden opponent information or send test-only state. Use real native controls or the same documented public actions; no new privilege or administration endpoint.

One attempt, 600 seconds total, at most 240 game mutation submissions including retries/CPU submissions/start/setup/initialize/equip. Reserve at least the final 60 seconds and 12 submissions for orderly stop and evidence. Each created game permits at most one explicit player surrender intent if needed; a lost reply may only retry the same ID within the total budget, never create a second surrender intent or another game. Use the owner-scoped existing active-room/snapshot reads for uncertainty, at most 40 read requests after normal UI initialization. Do not issue extra CPU actions merely to obtain a nicer result. Stop advancing to further stages on failure or inability to earn the prerequisite; record each not reached stage as NOT_RUN and leave no claim of full live trial acceptance. Retain test data unless a separately authorized cleanup says otherwise. Actual gateway/room/persistence outcomes are distinct from local 8-stage rehearsal, hosted CI, asset-byte proof and physical-device acceptance. A review may restrict this proposal; follow its exact saved decision and do not expand a consumed budget.
...
ガチャ入口の後続候補（UDL-062-gacha-entry-v1）は `docs/GACHA_ENTRY_DIET_20260914.md` に固定する。基準はクイズ03bc21f6ba927f71ce05efc438827d547993b21c。厳密assetは `app.js?v=20260914-6` / `style.css?v=20260914-3` / `standard-skill-registry.generated.js?v=20260914-2` / index.htmlの4assetで、実際の定義から生成した全5Lvの率を確認する。新候補固有の全テスト・Windows・真正Astraレビューの後も、親70e→b9→87→df62→03bcの公開確認とfresh main03bcを先に要求する。公開後は同SHA Pages成功、4asset厳密byte一致、write-free preflightのGACHA_ENTRY_DIET_REQUIRED・DIRECT_QUIZ_ENTRY_REQUIREDと既存終局contractを確認する。terminal-result.css / result-continuation.jsは `20260914-1` のまま。5選択と1枚/全部、全Lv比較、100枚上限、0枚、busy/pending/Lv/actionId/count/reload/retryの局所実browser・Windows証拠を保存する。DB/Edge/管理設定は各 `[]`、新画像0、確率・報酬・経済変更なし。追加profile・クイズ・抽選・対局・liveはこの手順では許可しない。親049承認や終了済みPages/CI/レビュー枠を流用せず、21cc/8dd9/GOVの具体的保留を迂回しない。下記のクイズ・終局assetは各固定候補の記録であり、新ガチャのmarkerへ書き換えない。
...
3. fresh mainを照合し、forceなしの同SHA main/Pages、既存preflightと候補の公開byte一致を確認する。配信確認だけを本番対局受入にしない。新プロフィール・対局・本番操作は本便の文書では0件、040の消費済み試行を再使用しない。
4. rollback時も記録した互換artifactと既存runbookを使い、保存済み対局・receiptを書き換えない。物理端末と実本番スキル観測は未実施ならNOT_RUN。
