...
再公開候補中。この便は新payloadを旧Edgeが拒否する一方、新Edgeは旧UIのoutgoing payloadを継続できるため、`alpha.4対応Edge → live canary → Pages`の順にする。後続の対戦報酬便、CPU台詞／地の文分離便、CPU選択前portrait便を維持したまま、通常1マス直接tapを追加する。Pages候補assetはonline app `app.js?v=20260910-25`、style `style.css?v=20260910-12`、intents `standard-online-skill-intents.js?v=20260910-2`、client `standard-online-client.js?v=20260910-1`、portrait `cpu-portraits.js?v=20260908-1`、Local bundle `app.bundle.js?v=20260910-9-666cd89bf05e`、corner target CSS `corner-target.css?v=20260910-1`。DB、migration、RPC、secret、cleanup scheduleは変更しない。
