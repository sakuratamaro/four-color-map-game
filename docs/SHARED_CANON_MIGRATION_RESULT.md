...
`node scripts/check-standard-decision-reconciliation.mjs --json` も51行・errors/warningsとも0でPASS。既存UDL-017／029／032の手動照合・main未統合は3件の要確認として維持しており、今回のPASSで解除していない。`git diff --check` もPASS。
