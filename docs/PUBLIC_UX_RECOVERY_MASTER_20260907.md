...
| 色ボタン内のおまけ色残数 | ユーザー確定 | IMPLEMENTING | random summaryだけが`bonusUsesRemaining`を表示し、`paletteControls`は色名だけ | UX/privacy・本worktree | 基本色重複・残0・一時色・prism・入替・補充matrix |
| 封印色の鍵と残期間 | ユーザー確定 | IMPLEMENTING | `availableColorChoices`が残0のおまけ色を除外するため、CSS上の鍵実装前にbutton自体が消える。封印期間も文言へ出していない | UX/privacy・本worktree | current-seat全所有色を表示、opponent palette非参照browser test |
