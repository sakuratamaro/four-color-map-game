...
### Current palette-change notice

UDL-20260909-035, UDL-20260909-038, and UDL-20260910-052 require the current seat's latest palette-change cause and actual remaining duration to be readable beside the board without an acknowledgment button. This is current status, not a replay of the historical event. Temporary pollution uses the matching slot's current `paletteDebuffs` remainder; after successful coloring reduces it, the notice updates, and it disappears when that effect expires. A still-matching color alone cannot revive an expired temporary notice. Permanent self/forced changes show their end-of-match duration only while that latest change still matches the current slot and no temporary effect supersedes it. Replaced changes, ended matches, foreign-seat/incoherent data, and hidden/non-battle views clear the notice.

Reopening battle or reloading restores a still-active status even when its event ID has already been recorded; identical polls do not rewrite the live announcement or duplicate the recorded event ID. The notice remains a short single latest-change summary, not an additional list of all active effects. Existing private change history is preserved unchanged, including its original duration. No engine duration, card, RNG, public projection, opponent-private information, database, or Edge behavior changes in this UI slice. `tests/standard-palette-notice-lifecycle.test.cjs` covers real-engine expiry plus current-projection reconciliation; scoped cases in `tests/standard-online-browser.test.cjs` cover Chrome/Edge display, reload, background/tab return, reduced motion, and narrow/short/desktop layouts. These local tests are not proof of production publication or physical-device acceptance.

## Neutral board affordances

UDL-20260909-039 and UDL-20260910-054, adopted through `ADD-20260912-BOARD-GRAYSCALE-AFFORDANCE`, reserve red, blue, yellow, and green inside the board for actual game colors. Candidate free microcells use brighter gray; selected free microcells use near-white. A partially occupied macro after Half Shift must retain every painted microcell's game color: no whole-macro translucent selection fill may whiten it. Thin neutral target/selection outlines and a white keyboard-focus outline with a dark edge preserve targeting and focus visibility; cyan dotted candidates, pink focus, and yellow/purple shift-band fills are not used. This changes presentation, not the legal candidate set, submitted region, keyboard/pointer selection, public contact feedback, zoom behavior, rules, or RNG.

`ADD-20260912-HALF-SHIFT-CANDIDATE-REPORT` remains a separately tracked candidate-mismatch report, not a proven rule bug or a fix inferred from recoloring the UI. `tests/standard-half-shift-candidate-parity.test.cjs` compares exact start and connected-prefix candidates, free-microcell commits, and outer-boundary exclusions against the authoritative engine before/after real Half Shift actions for six row/column/direction/edge scenarios and sizes 1–5 (60 conditions). A passing matrix means no mismatch reproduced in those conditions, not universal absence of a bug. `tests/standard-board-affordance.test.cjs` checks free-only fills and retained painted pixels at narrow widths; focused Chrome/Edge cases in `tests/standard-online-browser.test.cjs` verify real Canvas pixels, pointer/keyboard selection, and one submitted action against the same engine. Local evidence is not Windows CI, production publication, or physical-device acceptance.

## Compact active-match controls

UDL-20260910-054 and UDL-20260912-062, v13 `ADD-20260912-TURN-GUIDE-NO-ZOOM` / `ADD-20260912-REMOVE-DUPLICATE-MATCH-SETTINGS`, supersede the previous custom-zoom fallback and fold-settings-card design. Use one short outside-board yellow instruction with the existing selection count, clear and give controls adjacent above the board. Remove duplicate phase/role/detail instructions, custom zoom button/state/automatic fine-cell enlargement and permanent fallback prose. Native browser/OS zoom, current actor/phase, macro/micro pointer and keyboard selection, busy/pending/retry/terminal guards and focus cancellation remain. Remove only the duplicate settings display: required count stays at the action, colors/uses stay in the four-role palette, and initial palette/history, setup reveal, server state, seed/RNG and reload/rematch data remain unchanged. See `docs/UI_PLAY_SURFACE_20260912.md` v1.3 and `tests/standard-turn-guide-diet.test.cjs`; this is local implementation, not publication evidence.

## Compact hand information

UDL-20260912-063, v13 `ADD-20260912-HAND-COMPACT-INLINE-INFO`: the online own-hand remains3x2 with stable used slots. Omit only ordinary visual x1; retain rarity, used/repeated/debug/loan counts and catalog quantities. Information uses an independent44px inline card control plus the existing selected-target heading, not a large dedicated row. Reading information must not activate a card, consume inventory, alter the current target or send an action. Existing immediate/target activation steps and native disabled timing/category/pending guards remain unchanged. The assumption of a confirmation in the user wording is reconciled in `UI_PLAY_SURFACE_20260912.md` v1.4, not made a new universal mandatory confirmation. See `tests/standard-hand-compact.test.cjs` and browser layout/input coverage; no publication is asserted.
...
Presentation input is narrowed at the function boundary: `showContactReveal(contactColorCount, eventId)` receives only the post-commit public count and its validated public event ID after the caller checks saved, newly applied, non-replayed `CREATE_REGION`. It must not receive or inspect the session, authoritative root, private projection, palettes, bonus state, hands, private effects, unpublished loadouts, or RNG snapshot.
...
## Basic sound and vibration feedback

Online Standard offers two independent, explicit presentation preferences: synthesized effect sound and supported-device vibration. Both default to OFF and are stored locally in a strict versioned record. The later UDL062 Home-gear requirement places the single pair of keyboard-accessible checkboxes behind Home's initially closed settings button, superseding the old always-visible Home/Profile panel without resetting preferences. These preferences are independent from `prefers-reduced-motion`; changing motion preference cannot enable, disable, or rewrite either setting.

Sound uses only short Web Audio oscillator envelopes generated by product code. No network audio, embedded recording, game RNG, server write, action ID, or gameplay timer is involved. An `AudioContext` may be created or resumed only inside a trusted user pointer, keyboard, touch, or setting-change gesture after sound is ON. Reloaded ON preference therefore reads `次の操作で有効` until that gesture. Missing, blocked, suspended, or throwing audio APIs return silently and cannot reject or delay a game action. Turning sound OFF immediately stops every tracked playing or scheduled oscillator before suspending its context; an obsolete suspend or resume completion is revision-guarded and can only leave the next trusted-gesture retry armed.

Vibration calls `navigator.vibrate` only when its separate preference is ON, the API exists, the document is visible, and the browser is online. Turning vibration OFF safely calls `navigator.vibrate(0)` to cancel an active pattern. Unsupported and throwing implementations are no-ops. Vibration has no inferred fallback to sound, animation, or repeated visual motion.

Only three validated public presentation boundaries may emit feedback: a newly observed `CREATE_REGION` trace with contact count 2–4 uses its exact public `trace.eventId`; a visible opponent-to-local active-turn transition uses `${matchId}:${version}:turn:${seat}`; and a newly admitted terminal overlay uses `${matchId}:${version}:terminal:${winner}:${terminalReason}`. Contact, turn, victory, and defeat cues have fixed finite note and vibration patterns. Private projections, palettes, hands, unpublished loadouts, and hidden legality facts are never controller inputs.

The controller validates an allowlisted cue and a bounded public event ID, then asynchronously claims and records it in a 64-entry local ring while holding one origin-wide exclusive Web Lock. The lock encloses the full storage read-merge-write, so simultaneous same-ID tabs elect one presenter and simultaneous distinct IDs cannot overwrite one another. The app deliberately fire-and-forgets this presentation promise behind a local rejection sink; no game action, render, persistence, or timer awaits it. When Web Locks are unavailable, denied, or fail—or when the history write fails—the event is consumed locally but sound and vibration are suppressed, which is the safe no-duplicate degradation.

The claim completes before checking settings, unlock state, visibility, connectivity, or output API support. Consequently OFF, locked, hidden, offline, unsupported, or failed output is consumed rather than deferred. Polling, focus, reconnect, duplicate render, idempotent replay, reload, and a second same-origin controller cannot later replay that event. Storage failure retains in-memory dedupe for the page lifetime; malformed stored preferences or history fail closed.
