...
The controller validates an allowlisted cue and a bounded public event ID, then records the ID in a 64-entry local ring before checking settings, unlock state, visibility, connectivity, or API support. Consequently OFF, locked, hidden, offline, unsupported, or failed output is consumed rather than deferred. Polling, focus, reconnect, duplicate render, idempotent replay, reload, and a second same-origin controller cannot later replay that event. Storage failure retains in-memory dedupe for the page lifetime; malformed stored preferences or history fail closed.
