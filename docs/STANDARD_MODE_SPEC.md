...
Online Standard offers two independent, explicit presentation preferences: synthesized effect sound and supported-device vibration. Both default to OFF, are stored locally in a strict versioned record, and may be changed from the Home or Profile tab using ordinary keyboard-accessible checkboxes. These preferences are independent from `prefers-reduced-motion`; changing motion preference cannot enable, disable, or rewrite either setting.
