...
| `1a1d8f8` 持ち色の初期値・private変更履歴 | PASS（LOCAL） | 2026-09-10 | 新規対局の初期基本2色＋おまけ色を3色別のままauthoritative stateへ保存し、現在値とは別にcurrent-seat private projectionへ投影。後発の本人変更・持ち色汚染3種を最大12件のprivate履歴へactor、skill、slot、前後色、version付きで保持し、最新通知一回性と旧room fallbackを維持した。後発同色化でもslotとおまけ残数は統合しない。関連unit/static 194/194、390×844の実Chrome/Edge各1/1、生成済みLocal/Edge bundle parityを確認。main push、Edge deploy、live canary、Pagesは未実施 |
