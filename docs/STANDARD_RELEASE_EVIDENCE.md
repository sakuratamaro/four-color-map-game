...
| 現行公開Pages | PUBLIC_VERIFIED | 公開HEAD `26a4161`、Pages run `34065705946`成功。公開URLでapp v42、style v40、skill-intents v19を確認。390×844はhorizontal overflow 0、console warning/error 0、candidate preflight `ok:true` | 別々の二端末でカテゴリ制限、Shift、角膨張、音量・振動感を最終受入 |
...
| COLOR応答の長文撤去 | VERIFIED | B便→報酬便後続製品`608962e`。Online／Localから常設長文だけを撤去。静的85/85、390px Online Chrome/Edge各1/1、Local各3/3で通常色・色操作skill・任意投了、reload、保存境界、横overflow 0を確認。Local bundle SHA-256 `ad91938e65c4c7dd66f90c3efcef61ad54cb6d395364badd2c9f0d17a01ae79b` | 先行2便後にWindows gate、Pages公開、公開Online／Local実見 |
| 封印中color identity | VERIFIED | 先行3便後続製品`06d85a4`。4色の元背景・枠を維持して白ring・鍵・残り回数・disabledを重ねる。静的78/78、390px Chrome/Edge各1/1でcomputed style、基本／おまけ残0／prism、stale click write 0、非封印色保存、横overflow 0を確認 | 先行3便後にWindows gate、Pages公開、公開通常対戦で4色を実見 |
...
| `1a1d8f8`／統合`9f57c12` 持ち色の初期値・private変更履歴 | PASS（LOCAL） | 2026-09-10 | 新規対局の初期基本2色＋おまけ色を3色別のままauthoritative stateへ保存し、現在値とは別にcurrent-seat private projectionへ投影。後発の本人変更・持ち色汚染3種を最大12件のprivate履歴へactor、skill、slot、前後色、version付きで保持し、最新通知一回性と旧room fallbackを維持した。後発同色化でもslotとおまけ残数は統合しない。単独関連unit/static 194/194、390×844の実Chrome/Edge各1/1、生成済みLocal/Edge bundle parityを確認。先行6変更を含む累積統合はsource/static/docs 211/211、対象実Chrome/Edge各7/7で非回帰。main push、Edge deploy、live canary、Pagesは未実施 |
