...
| 現行公開Pages | PUBLIC_VERIFIED | 公開HEAD `4318793`、Pages run `34387630198`成功。公開URLでapp v10、style v6、candidate preflight `ok:true`。390×844はhorizontal overflow 0、console warning/error 0。通常CPU報酬`15→16`、ガチャ1枚後`16→15`、reload後も残数と獲得結果を維持 | 別々の二端末でカテゴリ制限、Shift、角膨張、音量・振動感を最終受入 |
...
- 現行公開製品`4bed50f`は全開始候補、各セル選択直後の接触演出、初期3色とprivate変更履歴、コスメ残高不足の事前抑止に加え、エリア二分を紫枠の通常1マスから即発動する。公開画面はapp v19／Local bundle v5で、実所有カードの対象2マス、盤面focus、別確定0、取消write 0、390px overflow 0、console 0を実見した。前便Edge sourceはDashboard配備後ZIPで候補とbyte完全一致し、基本7/7＋Runbook A 47/47。CLI未導入のためEdge control-plane id/versionはPENDINGで、物理二端末受入も未実施。B便のクイズ物理は公開PC・390pxで`PUBLIC_VERIFIED`。報酬P0は公開通常CPU戦で券`15→16→15`とreload後の結果維持まで確認したが、報酬強化UDL-046と物理端末の体感確認は別残件である。
