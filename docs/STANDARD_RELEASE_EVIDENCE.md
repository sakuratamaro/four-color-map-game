...
- 現行公開製品`d566d62`は全開始候補、各セル選択直後の接触演出に加え、初期3色distinct、現在値との分離、最大12件のseat-private変更履歴を含み、Pagesへ反映済み。Edge sourceはDashboard配備後ZIPで候補とbyte完全一致し、基本7/7＋Runbook A 47/47。公開画面はapp v17/style v11、旧対局fallback、390px overflow 0を実見した。CLI未導入のためEdge control-plane id/versionはPENDINGで、物理二端末受入も未実施。B便のクイズ物理は公開PC・390pxで`PUBLIC_VERIFIED`。報酬P0は公開通常CPU戦で券`15→16→15`とreload後の結果維持まで確認したが、報酬強化UDL-046と物理端末の体感確認は別残件である。
