...
- 現行公開製品`3ddac0c`は全開始候補、各セル選択直後の接触演出、初期3色とprivate変更履歴、コスメ残高不足の事前抑止、エリア二分の盤面直接指定に加え、CPU戦績10人をcompact一覧で常時表示する。公開画面はapp v20／progression CSS v1で、Chrome 390×844の2列、portrait ready 10/10、未対戦0戦、最長名、card／page overflow 0、console 0を実見した。Local bundleは前便v5のまま、DB／Edge／RPC／戦績集計／対戦通信は変更なし。前便Edge sourceはDashboard配備後ZIPで候補とbyte完全一致し、基本7/7＋Runbook A 47/47。CLI未導入のためEdge control-plane id/versionはPENDINGで、物理二端末受入も未実施。B便のクイズ物理は公開PC・390pxで`PUBLIC_VERIFIED`。報酬P0は公開通常CPU戦で券`15→16→15`とreload後の結果維持まで確認したが、報酬強化UDL-046と物理端末の体感確認は別残件である。
