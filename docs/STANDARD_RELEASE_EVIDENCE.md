...
| alpha.4彩色済みエリア角膨張 | VERIFIED | 製品`90e718b`、最終候補`f51118d`。生成bundleは2回目同一（Local SHA-256 `E4A6C9DB6FCB072929C2B352B52D8B7BF8C5693710B277C38ACB422CD13EAE05`、Edge `4AE11F822D5450E0876ED4C4C205A2912EAEEA4FD99042B47A1839D1B36E2841`）。focused 208/208、donor削除・外周clip追補50/50、Windows最終run `34060194268`のChrome/Edge両job成功。互換rollback`4f147b1`はGitHub保全。read-only診断はDB 16.7MB、disk 17%、CPU 2%、接続18/60、blocked 0、slot 2/2 active、DB契約72/72。Edge deployment 24へ2ファイル同時反映後、phase-less preflight、基本7/7、COLOR応答225/225が合格。DB、migration、RPC、secret変更なし | Pages公開とcandidate preflight。物理二端末はPENDING |
...
- 互換rollbackは`codex/standard-alpha4-compat-rollback-20260907@4f147b1`としてGitHubへ保全した。alpha.4対応bundleを残したまま新規matchだけalpha.3へ戻せる。active alpha.4 roomが存在し得るため、旧deployment 23への単純復帰は行わない。次の昇格条件はPages公開、candidate preflight、公開390px/console確認である。
