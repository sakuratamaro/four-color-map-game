...
- 現行公開製品`2cce7af`は全開始候補、各セル選択直後の接触演出、初期3色とprivate変更履歴に加え、残高不足のコスメ購入を`あとNコイン`で事前抑止し、保存済み売却後の残高を即時同期する。公開画面はapp v18/client v20260910-1、0コインの有料5件disabled、390px overflow 0を実見した。前便Edge sourceはDashboard配備後ZIPで候補とbyte完全一致し、基本7/7＋Runbook A 47/47。CLI未導入のためEdge control-plane id/versionはPENDINGで、物理二端末受入も未実施。B便のクイズ物理は公開PC・390pxで`PUBLIC_VERIFIED`。報酬P0は公開通常CPU戦で券`15→16→15`とreload後の結果維持まで確認したが、報酬強化UDL-046と物理端末の体感確認は別残件である。
