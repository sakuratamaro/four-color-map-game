...
- 現行公開製品`21c1e23`は基本palette torn snapshot拒否、クイズ選択肢compact巡回、CPU完了報酬の実所持突合、持ち色変更の変更元枠→変更先色UI、COLOR長文案内撤去を含み、Pagesへ反映済み。A便のsynthetic tornと持ち色変更action behaviorはformal browserで検証し、公開プロフィールにカードがないため本番actionを捏造していない。COLOR長文撤去はonline/localの公開DOMで通常操作残存とともに実見した。物理二端末受入は未実施。B便のクイズ物理は公開PC・390pxで`PUBLIC_VERIFIED`。報酬P0は公開通常CPU戦で券`15→16→15`とreload後の結果維持まで確認したが、報酬強化UDL-046と物理端末の体感確認は別残件である。
