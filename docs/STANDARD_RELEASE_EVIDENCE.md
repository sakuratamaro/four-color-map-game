...
| 製品コード・生成元 | PUBLIC_VERIFIED | alpha.3製品source `d627cd5`をmainへ適用し、起動修正込み`549e716`、favicon追補`df56432`。Local/Edge bundleは決定的再生成、配備済みEdge sourceはLF正規化後に候補と一致 | 次回engine変更時に再生成一致を確認 |
...
| alpha.4彩色済みエリア角膨張 | LOCAL_VERIFIED | 候補`90e718b`。生成bundleは2回目同一（Local SHA-256 `E4A6C9DB6FCB072929C2B352B52D8B7BF8C5693710B277C38ACB422CD13EAE05`、Edge `4AE11F822D5450E0876ED4C4C205A2912EAEEA4FD99042B47A1839D1B36E2841`）。focused 208/208、Chrome/Edge新UI各1/1、Edge contact-pressure 43/43、donor削除・外周clipを含む追補50/50。正式全製品試験は1036/1038で製品挙動は全合格、旧asset marker期待2件だけをv39/v41へ追随後に関連50/50。互換rollback`c33bb40`は77/77でGitHub保全。Windows run `34058941292`はChrome job `101555866919`、Edge job `101555867023`とも成功。DB、migration、RPC、secret変更なし | read-only資源診断→Edge先行canary→Pages。物理二端末はPENDING |
...
| Dashboard Advisor・使用量baseline | WATCH_PARTIAL | T0と2026-09-06のT+24hを正規化JSONへ保存。T+24hは15/37 metricを観測し、CPU 2%、RAM 62%、disk 17%、disk IO 1%、接続peak 20/60、Security errors 0。2026-09-07のEdge更新時にDashboardが複数resource逼迫を警告したが、公開canaryとpreflightは全て成功。原因・継続時間は未確定 | read-only診断と同一時間窓Dashboardを再取得し、推測値・0置換・即時cleanup/課金変更はしない |
...
| Edge Function更新 | PUBLIC_VERIFIED | deployment 23へalpha.3の`index.ts`と生成済みbundleを同時反映。配備ZIPのindexはLF正規化SHA `25F6045870BD583C21512184A1A3EFB53B47B428377B9D318A4F18530711FEA9`、bundleは`D6810705473E8761D24BA4871F1A45D407B636B4A88596DD8951536D018897E5`で候補と一致。基本7/7、COLOR追補263/263、LAB 23/23、CPU 108/108、preflight `ok:true`。更新前後のactive alpha.3 roomは0 | 物理端末でカテゴリ表示と救済判断の操作感を確認 |
