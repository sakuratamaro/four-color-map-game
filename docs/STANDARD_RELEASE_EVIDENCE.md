...
| Edge Function更新 | PUBLIC_VERIFIED | deployment 23へalpha.3の`index.ts`と生成済みbundleを同時反映。配備ZIPのindexはLF正規化SHA `25F6045870BD583C21512184A1A3EFB53B47B428377B9D318A4F18530711FEA9`、bundleは`D6810705473E8761D24BA4871F1A45D407B636B4A88596DD8951536D018897E5`で候補と一致。基本7/7、COLOR 105/105、LAB 23/23、CPU 108/108、preflight `ok:true`。更新前後のactive alpha.3 roomは0 | 物理端末でカテゴリ表示と救済判断の操作感を確認 |
...
- source側の非Playwrightは751/751。独立監査はfocused 232/232、Chrome/Edge全browser各77/77・skip 0、カテゴリ専用390pxケース両browser合格、bundle source包含Local 26/26・Edge 10/10、メモリ内再生成byte一致を確認し、P0/P1なし。P2はlive環境でのカテゴリ使用→同カテゴリrejectが未実施である点だけで、物理二端末も`NOT_RUN`のまま公開後受入へ残す。
