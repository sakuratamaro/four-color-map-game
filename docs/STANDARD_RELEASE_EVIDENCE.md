...
| CPU理由別ポートレート候補 | VERIFIED | `a9a1fc0`。10人・理由別49個の256×256 WebP（合計600,812 bytes）だけをruntime参照し、manifest、NOTICE、README、ゲーム内credit、画像失敗時CPU印fallbackを同梱。標準CPU敗北は理由別、human敗北とLAB CPU敗北は通常表情、PvPは非表示。静的＋製品契約73/73、CI契約はPlaywrightなしの環境依存1件を除く480/480、ローカル実Edge/Chrome各6/6。公開前のphase-less preflightは旧Pagesのportrait 404を安全に未導入判定し`ok:true`。engine、Edge、DB、migration、RPC、secret変更なし | Windows Chrome/Edge正式gate、main、Pages、公開49 asset・390px・console確認 |
