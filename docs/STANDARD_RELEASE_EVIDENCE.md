...
| alpha.4彩色済みエリア角膨張 | VERIFIED | engine製品`90e718b`、Edge/生成bundle/互換は検証済み。ユーザー決定どおりカード→公開microcell→即発動へ修正し、旧mode/番号候補/別確定UIを撤去。focused非browser 138/138、390pxの実Edge/Chrome各6/6、独立再監査P0/P1なし。対象外通信0、preparedOutgoing継続とpointer lock、retry同一性、server不成立後の44px再選択を確認。互換rollback`4d2f6ff`はGitHub保全。Edge deployment 24は基本7/7＋COLOR応答225/225、配備ZIP SHA一致。DB、migration、RPC、secret変更なし | 正式Windows CI、Pages公開、candidate preflight、公開390px/console。物理二端末はPENDING |
...
- 互換rollbackは`codex/standard-alpha4-compat-rollback-20260907@4d2f6ff`としてGitHubへ保全した。alpha.4対応bundleを残したまま新規matchだけalpha.3へ戻せる。active alpha.4 roomが存在し得るため、旧deployment 23への単純復帰は行わない。次の昇格条件はPages公開、candidate preflight、公開390px/console確認である。
