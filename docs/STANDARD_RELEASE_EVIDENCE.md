...
- 現行公開製品`e50044a`は全開始候補、各セル選択直後の接触演出、初期3色とprivate変更履歴、コスメ残高不足の事前抑止、エリア二分の盤面直接指定、CPU戦績10人のcompact一覧に加え、CPU封印skillを公開応答候補の圧力で判断する。公開画面はapp v20／progression CSS v1、Local bundle v6。EdgeはDashboard配備後のindex／bundle全文が候補と完全一致し、JWT ON、基本7/7＋CPU live 107/107。Pages `34428346200`、candidate preflight `ok:true`、公開Chromeの表示正常・console 0を確認した。CLI未導入のためEdge control-plane id/versionはPENDINGで、物理二端末受入も未実施。B便のクイズ物理は公開PC・390pxで`PUBLIC_VERIFIED`。報酬P0は公開通常CPU戦で券`15→16→15`とreload後の結果維持まで確認したが、報酬強化UDL-046と物理端末の体感確認は別残件である。
