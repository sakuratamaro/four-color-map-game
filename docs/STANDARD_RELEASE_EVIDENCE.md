...
- 現行公開製品`c36bd18`（製品`a60aee3`）は全開始候補、各セル選択直後の接触演出、初期3色とprivate変更履歴、コスメ残高不足の事前抑止、エリア二分の盤面直接指定、CPU戦績10人のcompact一覧、CPU封印skill局面判断に加え、migration後の確定回答だけによるクイズ全体＋Lv.1〜5正答率を表示する。公開画面はapp v21／progression CSS v2、Local bundle v7。DB正答率契約true、focused live 47/47、Pages `34433653551`、candidate preflight `ok:true`、公開Chrome 390pxの6件・2列・overflow 0・console 0を確認した。Edge sourceは前便と同一で、CLI未導入のためcontrol-plane id/versionはPENDING。物理二端末受入、報酬強化UDL-046、実振動端末の体感確認は別残件である。
