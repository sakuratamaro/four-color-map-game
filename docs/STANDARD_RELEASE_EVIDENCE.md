...
- 現行公開製品`816f51e`は基本palette torn snapshot拒否、クイズ選択肢compact巡回、CPU完了報酬の実所持突合、持ち色変更の変更元枠→変更先色UI、COLOR長文案内撤去、封印中の元色保持を含み、Pagesへ反映済み。封印色は正式Chrome/Edgeで行動境界を確認し、公開画面ではapp v14/style v10、390px overflow 0、CSSOMの4色固有値とsealed opacity 1を実見した。公開プロフィールに実封印状態がないため本番対局を捏造していない。物理二端末受入は未実施。B便のクイズ物理は公開PC・390pxで`PUBLIC_VERIFIED`。報酬P0は公開通常CPU戦で券`15→16→15`とreload後の結果維持まで確認したが、報酬強化UDL-046と物理端末の体感確認は別残件である。
